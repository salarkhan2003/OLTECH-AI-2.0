...
export default function TasksPage() {
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter(t =>
      (!filterAssignee || (t as Task).assigned_to === filterAssignee) &&
      (!filterPriority || (t as Task).priority === filterPriority) &&
      (!filterProject || (t as Task).project_id === filterProject)
    );
  ...
  const taskCounts = useMemo(() => {
    ...
    statusColumns.forEach(col => {
      counts[col.key] = filteredTasks.filter(t => (t as Task).status === col.key).length;
    });
    return counts;
  }, [filteredTasks]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCreatedTaskSummary({
        ...
        assignee: profiles.find(p => (p as Profile).id === newTask.assigned_to)?.name || "Unassigned",
        project: projects.find(p => (p as Project).id === newTask.project_id)?.name || "--",
      });
      setShowSummary(true);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData = {
        ...editTask,
        description: (editTask as Task).description || null,
        priority: (editTask as Task).priority || null,
        due_date: (editTask as Task).due_date || null,
        status: (editTask as Task).status || null,
        project_id: (editTask as Task).project_id || null,
        assigned_to: (editTask as Task).assigned_to || null,
      };
      const { error } = await supabase.from("tasks").update(updateData).eq("id", (editTask as Task).id);
      if (error) throw error;
      setEditTask(null);
    } catch (err: any) {
  ...
  async function handleStatusChange(taskId: string, newStatus: string) {
    ...
    try {
      ...
      setTasks(prev => prev.map(t => (t as Task).id === taskId ? { ...t, status: newStatus } : t));
    } catch (err: any) {
        ...
        <main className="p-8 min-h-screen bg-background text-foreground">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex gap-2 flex-wrap">
                <select className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={filterAssignee} onChange={e => setFilterAssignee(e.target.value)} title="Filter by assignee">
                  ...
                  {profiles.map(p => <option key={(p as Profile).id} value={(p as Profile).id}>{(p as Profile).name}</option>)}
                </select>
                ...
                <select className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={filterProject} onChange={e => setFilterProject(e.target.value)} title="Filter by project">
                  ...
                  {projects.map(p => <option key={(p as Project).id} value={(p as Project).id}>{(p as Project).name}</option>)}
                </select>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {statusColumns.map((col, idx) => {
              const colTasks = filteredTasks.filter(t => (t as Task).status === col.key);
              const colGradient = [
                ...
                <div key={col.key} className={`bg-gradient-to-br ${colGradient} rounded-2xl shadow-2xl p-4 md:p-6 flex flex-col min-h-[500px] border-2 border-blue-100 dark:border-blue-900 text-card-foreground mb-4 md:mb-0`}>
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    {colTasks.slice(0, MAX_VISIBLE).map(task => {
                      const assignee = profiles.find(p => (p as Profile).id === (task as Task).assigned_to);
                      const project = projects.find(p => (p as Project).id === (task as Task).project_id);
                      return (
                        <div key={(task as Task).id} className="bg-white dark:bg-background rounded-xl p-4 shadow-lg flex flex-col gap-2 relative group border border-blue-100 dark:border-blue-900 min-w-0 w-full touch-manipulation">
                          <div className="flex items-center justify-between">
                            <div className="font-bold text-lg truncate" title={(task as Task).title}>{(task as Task).title}</div>
                            <button className="p-1 rounded hover:bg-blue-100" onClick={() => setOpenDropdown(openDropdown === (task as Task).id ? null : (task as Task).id)} title="Task actions" aria-label="Task actions">
                              <MoreVertical className="w-5 h-5 text-blue-700" />
                            </button>
                            {openDropdown === (task as Task).id && (
                              <div className="absolute right-2 top-10 z-50 bg-card border rounded shadow-lg flex flex-col min-w-[120px] animate-fade-in text-card-foreground">
                                ...
                                <button className="px-4 py-2 hover:bg-red-50 text-left text-red-600" onClick={() => { handleDelete((task as Task).id); setOpenDropdown(null); }} disabled={deleteLoading === (task as Task).id}>{deleteLoading === (task as Task).id ? 'Deleting...' : 'Delete'}</button>
                              </div>
                            )}
                          </div>
                          {(task as Task).description && <div className="text-sm text-gray-700 truncate" title={(task as Task).description}>{(task as Task).description}</div>}
                          <div className="flex gap-2 text-xs mt-1 items-center">
                            <span className={`px-2 py-0.5 rounded font-bold ${getPriorityClass((task as Task).priority)}`}>{(task as Task).priority?.toUpperCase()}</span>
                            <span className="text-gray-600">{formatDueDate((task as Task).due_date)}</span>
                          </div>
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <label htmlFor={`status-select-${(task as Task).id}`} className="text-xs font-bold">Status:</label>
                            <select
                              id={`status-select-${(task as Task).id}`}
                              ...
                              onChange={e => handleStatusChange((task as Task).id, e.target.value)}
                              disabled={statusLoading === (task as Task).id}
                              title="Change status"
                            ...
                            {statusLoading === (task as Task).id && <span className="ml-2 text-xs text-blue-500 animate-pulse">Saving...</span>}
                          </div>
                        </div>
                      );
                    })}
          ...
          {showSummary && createdTaskSummary && (
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-card p-6 rounded shadow w-full max-w-md flex flex-col gap-4 text-card-foreground">
                ...
                <div className="flex flex-col gap-2">
                  <div><span className="font-semibold">Title:</span> {(createdTaskSummary as Task).title}</div>
                  {(createdTaskSummary as Task).due_date && <div><span className="font-semibold">Deadline:</span> {(createdTaskSummary as Task).due_date}</div>}
                  <div><span className="font-semibold">Created By:</span> {(createdTaskSummary as Profile).name}</div>
                  <div><span className="font-semibold">Assignee:</span> {(createdTaskSummary as Task).assigned_to ? (profiles.find(p => (p as Profile).id === (createdTaskSummary as Task).assigned_to)?.name || "Unassigned") : "Unassigned"}</div>
                  <div><span className="font-semibold">Project:</span> {(createdTaskSummary as Task).project_id ? (projects.find(p => (p as Project).id === (createdTaskSummary as Task).project_id)?.name || "--") : "--"}</div>
                  {(createdTaskSummary as Task).description && <div><span className="font-semibold">Description:</span> {(createdTaskSummary as Task).description}</div>}
                  {(createdTaskSummary as Task).priority && <div><span className="font-semibold">Priority:</span> {(createdTaskSummary as Task).priority}</div>}
                  {(createdTaskSummary as Task).status && <div><span className="font-semibold">Status:</span> {(createdTaskSummary as Task).status}</div>}
                </div>
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-card p-6 rounded shadow w-full max-w-md flex flex-col gap-4 text-card-foreground">
                ...
                <form className="flex flex-col gap-3" onSubmit={handleEdit}>
                  <Input placeholder="Title" value={(editTask as Task).title || ""} onChange={e => setEditTask((t: Task) => ({ ...t, title: e.target.value }))} required />
                  <Input placeholder="Description (optional)" value={(editTask as Task).description || ""} onChange={e => setEditTask((t: Task) => ({ ...t, description: e.target.value }))} />
                  <label htmlFor="edit-priority-select" className="text-sm font-medium">Priority (optional)</label>
                  <select id="edit-priority-select" title="Priority" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={(editTask as Task).priority || ""} onChange={e => setEditTask((t: Task) => ({ ...t, priority: e.target.value }))}>
                    <option value="">No Priority</option>
                    {priorities.map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
                  </select>
                  <label htmlFor="edit-deadline-input" className="text-sm font-medium">Deadline (optional)</label>
                  <Input id="edit-deadline-input" type="date" value={(editTask as Task).due_date || ""} onChange={e => setEditTask((t: Task) => ({ ...t, due_date: e.target.value }))} className="bg-background text-foreground dark:bg-background dark:text-foreground" />
                  <label htmlFor="edit-status-select" className="text-sm font-medium">Status (optional)</label>
                  <select id="edit-status-select" title="Status" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={(editTask as Task).status || ""} onChange={e => setEditTask((t: Task) => ({ ...t, status: e.target.value }))}>
                    <option value="">No Status</option>
                    {statusColumns.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
                  </select>
                  <label htmlFor="edit-project-select" className="text-sm font-medium">Project (optional)</label>
                  <select id="edit-project-select" title="Project" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={(editTask as Task).project_id || ""} onChange={e => setEditTask((t: Task) => ({ ...t, project_id: e.target.value }))}>
                    <option value="">No Project</option>
                    {projects.map(p => <option key={(p as Project).id} value={(p as Project).id}>{(p as Project).name}</option>)}
                  </select>
                  <label htmlFor="edit-assignee-select" className="text-sm font-medium">Assignee (optional)</label>
                  <select id="edit-assignee-select" title="Assignee" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={(editTask as Task).assigned_to || ""} onChange={e => setEditTask((t: Task) => ({ ...t, assigned_to: e.target.value }))}>
                    <option value="">Unassigned</option>
                    {profiles.map(p => <option key={(p as Profile).id} value={(p as Profile).id}>{(p as Profile).name}</option>)}
                  </select>
                  {error && <div className="text-red-600 text-sm">{error}</div>}
