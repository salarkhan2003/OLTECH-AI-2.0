...
export default function TasksPage() {
  ...
  const [createdTaskSummary, setCreatedTaskSummary] = useState<any>(null);
  ...
  const taskCounts = useMemo(() => {
    ...
    statusColumns.forEach(col => {
      counts[col.key] = filteredTasks.filter(t => t.status === col.key).length;
    });
    return counts;
  }, [filteredTasks]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCreatedTaskSummary({
        ...
        assignee: profiles.find(p => p.id === newTask.assigned_to)?.name || "Unassigned",
                ...
                <div key={col.key} className={`bg-gradient-to-br ${colGradient} rounded-2xl shadow-2xl p-4 md:p-6 flex flex-col min-h-[500px] border-2 border-blue-100 dark:border-blue-900 text-card-foreground mb-4 md:mb-0`}>
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    {colTasks.slice(0, MAX_VISIBLE).map(task => {
                      ...
                      return (
                        <div key={task.id} className="bg-white dark:bg-background rounded-xl p-4 shadow-lg flex flex-col gap-2 relative group border border-blue-100 dark:border-blue-900 min-w-0 w-full touch-manipulation">
                          <div className="flex items-center justify-between">
                            <div className="font-bold text-lg truncate" title={task.title}>{task.title}</div>
                            <button className="p-1 rounded hover:bg-blue-100" onClick={() => setOpenDropdown(openDropdown === task.id ? null : task.id)} title="Task actions" aria-label="Task actions">
                            ...
                            {openDropdown === task.id && (
                              <div className="absolute right-2 top-10 z-50 bg-card border rounded shadow-lg flex flex-col min-w-[120px] animate-fade-in text-card-foreground">
                                ...
                                <button className="px-4 py-2 hover:bg-red-50 text-left text-red-600" onClick={() => { handleDelete(task.id); setOpenDropdown(null); }} disabled={deleteLoading === task.id}>{deleteLoading === task.id ? 'Deleting...' : 'Delete'}</button>
                              </div>
                            )}
                          </div>
                          {task.description && <div className="text-sm text-gray-700 truncate" title={task.description}>{task.description}</div>}
                          <div className="flex gap-2 text-xs mt-1 items-center">
                            <span className={`px-2 py-0.5 rounded font-bold ${getPriorityClass(task.priority)}`}>{task.priority?.toUpperCase()}</span>
                            <span className="text-gray-600">{formatDueDate(task.due_date)}</span>
                          </div>
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <label htmlFor={`status-select-${task.id}`} className="text-xs font-bold">Status:</label>
                            <select
                            ...
                            {statusLoading === task.id && <span className="ml-2 text-xs text-blue-500 animate-pulse">Saving...</span>}
                          </div>
                        </div>
                      );
                    })}
          ...
          {showSummary && createdTaskSummary && (
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-card p-6 rounded shadow w-full max-w-md flex flex-col gap-4 text-card-foreground">
                ...
                <div className="flex flex-col gap-2">
                  <div><span className="font-semibold">Title:</span> {createdTaskSummary.title}</div>
                  {createdTaskSummary.due_date && <div><span className="font-semibold">Deadline:</span> {createdTaskSummary.due_date}</div>}
                  <div><span className="font-semibold">Created By:</span> {createdTaskSummary.creator}</div>
                  <div><span className="font-semibold">Assignee:</span> {createdTaskSummary.assignee}</div>
                  <div><span className="font-semibold">Project:</span> {createdTaskSummary.project}</div>
                  {createdTaskSummary.description && <div><span className="font-semibold">Description:</span> {createdTaskSummary.description}</div>}
                  {createdTaskSummary.priority && <div><span className="font-semibold">Priority:</span> {createdTaskSummary.priority}</div>}
                  {createdTaskSummary.status && <div><span className="font-semibold">Status:</span> {createdTaskSummary.status}</div>}
                </div>
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-card p-6 rounded shadow w-full max-w-md flex flex-col gap-4 text-card-foreground">
                ...
                <form className="flex flex-col gap-3" onSubmit={handleEdit}>
                  <Input placeholder="Title" value={editTask.title || ""} onChange={e => setEditTask((t: any) => ({ ...t, title: e.target.value }))} required />
                  <Input placeholder="Description (optional)" value={editTask.description || ""} onChange={e => setEditTask((t: any) => ({ ...t, description: e.target.value }))} />
                  <label htmlFor="edit-priority-select" className="text-sm font-medium">Priority (optional)</label>
                  <select id="edit-priority-select" title="Priority" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={editTask.priority || ""} onChange={e => setEditTask((t: any) => ({ ...t, priority: e.target.value }))}>
                    <option value="">No Priority</option>
                    {priorities.map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
                  </select>
                  <label htmlFor="edit-deadline-input" className="text-sm font-medium">Deadline (optional)</label>
                  <Input id="edit-deadline-input" type="date" value={editTask.due_date || ""} onChange={e => setEditTask((t: any) => ({ ...t, due_date: e.target.value }))} className="bg-background text-foreground dark:bg-background dark:text-foreground" />
                  <label htmlFor="edit-status-select" className="text-sm font-medium">Status (optional)</label>
                  <select id="edit-status-select" title="Status" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={editTask.status || ""} onChange={e => setEditTask((t: any) => ({ ...t, status: e.target.value }))}>
                    <option value="">No Status</option>
                    {statusColumns.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
                  </select>
                  <label htmlFor="edit-project-select" className="text-sm font-medium">Project (optional)</label>
                  <select id="edit-project-select" title="Project" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={editTask.project_id || ""} onChange={e => setEditTask((t: any) => ({ ...t, project_id: e.target.value }))}>
                    <option value="">No Project</option>
                    {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                  <label htmlFor="edit-assignee-select" className="text-sm font-medium">Assignee (optional)</label>
                  <select id="edit-assignee-select" title="Assignee" className="border rounded p-2 bg-background text-foreground dark:bg-background dark:text-foreground" value={editTask.assigned_to || ""} onChange={e => setEditTask((t: any) => ({ ...t, assigned_to: e.target.value }))}>
                    <option value="">Unassigned</option>
                    {profiles.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                  {error && <div className="text-red-600 text-sm">{error}</div>}
